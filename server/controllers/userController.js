...
const { User, UserItem } = require('../models/models');
...
class UserController {
  async registration(req, res, next) {
    ...
    const user = await User.create({email: email, login: login, role: role, password: hashPassword})
    const userStore = await UserItem.create({userId: user.id})
